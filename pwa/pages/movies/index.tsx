...
import { Movie } from '@damianchojnacki/cinema'
...
const PageList: NextComponentType = () => {
  ...
  const { data, fetchNextPage } = useInfiniteQuery({
    ...
    getNextPageParam: (data) => data?.links?.next?.split('?page=')[1],
  })

  const movies = useMemo(() => data?.pages.flatMap((page) => page?.data).filter((movie) => movie), [data])
